...
export class SRDWorkflow {
  ...
  private async stepRelevance() {
    this.state.currentStep = 'relevance';
    this.callbacks.onStatus('relevance', 'srd-orchestrator', 'Checking section relevance...');
  ...
  private async handleRelevanceResponse() {
    ...
    if (response === 'NO' || response === 'N') {
      this.state.answers.applicable = false;
      await this.persist('terminated');
      this.callbacks.onStatus('done', 'srd-orchestrator', 'Section marked as NOT APPLICABLE');
      this.callbacks.onDone(0);
      return;
    }

    this.state.answers.applicable = true;
    this.callbacks.onStatus('generate_questions', 'srd-orchestrator', 'Starting discovery...');
    await this.stepGenerateQuestions();
  }

  private async stepGenerateQuestions() {
    this.state.currentStep = 'generate_questions';
    this.callbacks.onStatus('generate_questions', 'srd-writer', 'Generating questions...');
    this.state.agentCalls.writer++;
    ...
    this.state.pendingQuestions = questions;
    this.callbacks.onStatus('negotiate_answers', 'srd-negotiator', 'Negotiating answers...');
    await this.stepNegotiateAnswers();
  }
  ...
  private async handleNegotiateAnswersResponse() {
    ...
    this.callbacks.onStatus('expectations', 'srd-orchestrator', 'Asking about expectations...');
    await this.stepExpectations();
  }
  ...
  private async handleExpectationsResponse() {
    this.state.answers.expectations = (this.context.userResponse || '').toString();
    this.callbacks.onStatus('suggest', 'srd-writer', 'Suggesting structure options...');
    await this.stepSuggest();
  }
  ...
  private async handleSuggestResponse() {
    this.state.answers.structureChoice = (this.context.userResponse || 'A').toString();
    this.callbacks.onStatus('generate', 'srd-writer', 'Generating section content...');
    await this.stepGenerate();
  }
  ...
  private async stepGenerate() {
    ...
    this.callbacks.onStatus('validate', 'srd-validator', 'Running quality checks...');
    await this.stepValidate();
  }
  ...
  private async stepValidate() {
    ...
    if (hasBlocking) {
      this.callbacks.onStatus('negotiate_fixes', 'srd-negotiator', 'Proposing fixes for findings...');
      await this.stepNegotiateFixes();
    } else {
      this.callbacks.onStatus('review', 'srd-orchestrator', 'Presenting draft for review...');
      await this.stepReview();
    }
  }
  ...
  private async handleDirectValidatorResponse() {
    ...
    if (hasAcceptedFixes) {
      this.callbacks.onStatus('fix', 'srd-writer', 'Applying selected fixes...');
      await this.stepFix();
    } else {
      this.callbacks.onStatus('review', 'srd-orchestrator', 'Presenting draft for review...');
      await this.stepReview();
  ...
  private async stepFix() {
    ...
    this.callbacks.onStatus('validate', 'srd-validator', 'Re-checking after fixes...');
    await this.stepValidate();
  }
  ...
  private async handleReviewResponse() {
    ...
    if (action === 'approve' || action === 'yes') {
      ...
      const tokensUsed =
        this.state.agentCalls.writer + this.state.agentCalls.negotiator + this.state.agentCalls.validator;
      this.callbacks.onStatus('done', 'srd-orchestrator', 'Section approved and locked');
      this.callbacks.onDone(tokensUsed);
    } else if (action === 'revise' || action === 'no') {
      this.state.revisionCount++;
...
export class TCWorkflow {
  ...
  private async stepRelevance() {
    this.state.currentStep = 'relevance';
    this.callbacks.onStatus('relevance', 'tc-orchestrator', 'Checking section relevance...');
  ...
  private async handleRelevanceResponse() {
    ...
    if (response === 'NO' || response === 'N') {
      this.state.answers.applicable = false;
      await this.persist('terminated');
      this.callbacks.onStatus('done', 'tc-orchestrator', 'Section marked as NOT APPLICABLE');
      this.callbacks.onDone(0);
      return;
    }

    this.state.answers.applicable = true;
    this.callbacks.onStatus('generate_questions', 'tc-orchestrator', 'Starting discovery...');
    await this.stepGenerateQuestions();
  }

  private async stepGenerateQuestions() {
    this.state.currentStep = 'generate_questions';
    this.callbacks.onStatus('generate_questions', 'tc-writer', 'Generating questions...');
    this.state.agentCalls.writer++;
    ...
    this.state.pendingQuestions = questions;
    this.callbacks.onStatus('negotiate_answers', 'tc-negotiator', 'Negotiating answers...');
    await this.stepNegotiateAnswers();
  }
  ...
  private async handleNegotiateAnswersResponse() {
    ...
    this.callbacks.onStatus('expectations', 'tc-orchestrator', 'Asking about expectations...');
    await this.stepExpectations();
  }
  ...
  private async handleExpectationsResponse() {
    this.state.answers.expectations = (this.context.userResponse || '').toString();
    this.callbacks.onStatus('suggest', 'tc-writer', 'Suggesting structure options...');
    await this.stepSuggest();
  }
  ...
  private async handleSuggestResponse() {
    this.state.answers.structureChoice = (this.context.userResponse || 'A').toString();
    this.callbacks.onStatus('generate', 'tc-writer', 'Generating section content...');
    await this.stepGenerate();
  }
  ...
  private async stepGenerate() {
    ...
    this.callbacks.onStatus('validate', 'tc-validator', 'Running quality checks...');
    await this.stepValidate();
  }
  ...
  private async stepValidate() {
    ...
    if (hasBlocking) {
      this.callbacks.onStatus('negotiate_fixes', 'tc-negotiator', 'Proposing fixes for findings...');
      await this.stepNegotiateFixes();
    } else {
      this.callbacks.onStatus('review', 'tc-orchestrator', 'Presenting draft for review...');
      await this.stepReview();
    }
  }
  ...
  private async stepFix() {
    ...
    this.callbacks.onStatus('validate', 'tc-validator', 'Re-checking after fixes...');
    await this.stepValidate();
  }
  ...
  private async handleReviewResponse() {
    ...
    if (action === 'approve' || action === 'yes') {
      ...
      const tokensUsed =
        this.state.agentCalls.writer + this.state.agentCalls.negotiator + this.state.agentCalls.validator;
      this.callbacks.onStatus('done', 'tc-orchestrator', 'Section approved and locked');
      this.callbacks.onDone(tokensUsed);
    } else if (action === 'revise' || action === 'no') {
      this.state.revisionCount++;
...
export class SRSFEWorkflow {
  ...
  private async stepRelevance() {
    this.state.currentStep = 'relevance';
    this.callbacks.onStatus('relevance', 'srs-fe-orchestrator', 'Checking section relevance...');
  ...
  private async handleRelevanceResponse() {
    ...
    if (response === 'NO' || response === 'N') {
      this.state.answers.applicable = false;
      await this.persist('terminated');
      this.callbacks.onStatus('done', 'srs-fe-orchestrator', 'Section marked as NOT APPLICABLE');
      this.callbacks.onDone(0);
      return;
    }

    this.state.answers.applicable = true;
    this.callbacks.onStatus('generate_questions', 'srs-fe-orchestrator', 'Starting discovery...');
    await this.stepGenerateQuestions();
  }

  private async stepGenerateQuestions() {
    this.state.currentStep = 'generate_questions';
    this.callbacks.onStatus('generate_questions', 'srs-fe-writer', 'Generating questions...');
    this.state.agentCalls.writer++;
    ...
    this.state.pendingQuestions = questions;
    this.callbacks.onStatus('negotiate_answers', 'srs-fe-negotiator', 'Negotiating answers...');
    await this.stepNegotiateAnswers();
  }
  ...
  private async handleNegotiateAnswersResponse() {
    ...
    this.callbacks.onStatus('expectations', 'srs-fe-orchestrator', 'Asking about expectations...');
    await this.stepExpectations();
  }
  ...
  private async handleExpectationsResponse() {
    this.state.answers.expectations = (this.context.userResponse || '').toString();
    this.callbacks.onStatus('suggest', 'srs-fe-writer', 'Suggesting structure options...');
    await this.stepSuggest();
  }
  ...
  private async handleSuggestResponse() {
    this.state.answers.structureChoice = (this.context.userResponse || 'A').toString();
    this.callbacks.onStatus('generate', 'srs-fe-writer', 'Generating section content...');
    await this.stepGenerate();
  }
  ...
  private async stepGenerate() {
    ...
    this.callbacks.onStatus('validate', 'srs-fe-validator', 'Running quality checks...');
    await this.stepValidate();
  }
  ...
  private async stepValidate() {
    ...
    if (hasBlocking) {
      this.callbacks.onStatus('negotiate_fixes', 'srs-fe-negotiator', 'Proposing fixes for findings...');
      await this.stepNegotiateFixes();
    } else {
      this.callbacks.onStatus('review', 'srs-fe-orchestrator', 'Presenting draft for review...');
      await this.stepReview();
    }
  }
  ...
  private async handleDirectValidatorResponse() {
    ...
    if (hasAcceptedFixes) {
      this.callbacks.onStatus('fix', 'srs-fe-writer', 'Applying selected fixes...');
      await this.stepFix();
    } else {
      this.callbacks.onStatus('review', 'srs-fe-orchestrator', 'Presenting draft for review...');
      await this.stepReview();
  ...
  private async stepFix() {
    ...
    this.callbacks.onStatus('validate', 'srs-fe-validator', 'Re-checking after fixes...');
    await this.stepValidate();
  }
  ...
  private async handleReviewResponse() {
    ...
    if (action === 'approve' || action === 'yes') {
      ...
      const tokensUsed =
        this.state.agentCalls.writer + this.state.agentCalls.negotiator + this.state.agentCalls.validator;
      this.callbacks.onStatus('done', 'srs-fe-orchestrator', 'Section approved and locked');
      this.callbacks.onDone(tokensUsed);
    } else if (action === 'revise' || action === 'no') {
      this.state.revisionCount++;
