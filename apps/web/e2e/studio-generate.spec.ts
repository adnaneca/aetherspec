...
async function resetStep(page: Page, stepNumber: number) {
  ...
  const resetResp = await page.request.patch(`${GATEWAY_URL}/api/document/${brsDoc.id}/step/${stepNumber}`, {
...
test.describe('Aether Studio interactive BRS workflow', () => {
  test('Generate Section starts interactive workflow and shows Question card', async ({ page }) => {
    ...
    await expect(page.locator('text=relevance').first()).toBeVisible({ timeout: 30000 });

    // Question card for relevance check
    await expect(page.locator('text=Is this section applicable').first()).toBeVisible({ timeout: 60000 });
    await expect(page.getByRole('button', { name: /Submit Answers/i })).toBeVisible();

    printLogs(consoleLogs);
  });

  test('Interactive workflow: relevance → suggestions → expectations → options → review', async ({ page }) => {
    ...
    async function submitRelevanceAnswer() {
      ...
      const relevanceCard = page.locator('[data-testid="workflow-card"]').last();
      ...
      const submitBtn = relevanceCard.getByRole('button', { name: /Submit Answers/i });
    ...
    await expect(page.locator('text=Negotiator proposes answers').first()).toBeVisible({ timeout: 120000 });

    // ── Suggestions: accept first suggestion ──
    const suggestionCard = page.locator('[data-testid="workflow-card"]').last();
    await suggestionCard.getByRole('button', { name: /^Accept$/i }).first().click();
    await suggestionCard.getByRole('button', { name: /Submit Answers/i }).click();

    // ── Expectations: fill and submit ──
    await expect(page.locator('text=Asking about expectations').first()).toBeVisible({ timeout: 60000 });
    const expectationsCard = page.locator('[data-testid="workflow-card"]').last();
    const expTextareas = expectationsCard.locator('textarea');
    await expect.poll(async () => await expTextareas.count(), { timeout: 60000 }).toBeGreaterThanOrEqual(1);
    for (let i = 0; i < await expTextareas.count(); i++) {
      await expTextareas.nth(i).fill(`Expectation ${i + 1}`);
    }
    await expectationsCard.getByRole('button', { name: /Submit Answers/i }).click();

    // ── Options: select Option A ──
    await expect(page.locator('text=Structure Options').first()).toBeVisible({ timeout: 60000 });
    const optionCard = page.locator('[data-testid="workflow-card"]').last();
    await optionCard.locator('button').filter({ hasText: /^Option A/i }).first().click();

    // ── Generation / validation / review ──
    await expect(page.locator('text=Generating section content').first()).toBeVisible({ timeout: 60000 });
    await expect(page.locator('text=Draft ready for review').first()).toBeVisible({ timeout: 300000 });

    // Editor should have streamed content
    await page.getByRole('button', { name: /Split/i }).click();
    await expect.poll(async () => (await page.locator('textarea').first().inputValue()).length, {
      timeout: 120000,
    }).toBeGreaterThan(100);

    // Review card has approve button
    const reviewCard = page.locator('[data-testid="workflow-card"]').last();
    await reviewCard.getByRole('button', { name: /Approve/i }).first().click();
    await reviewCard.getByRole('button', { name: /Confirm: Approve \u0026 Lock Section/i }).click();

    // Wait for done status
    await expect(page.locator('text=Section approved and locked').first()).toBeVisible({ timeout: 60000 });
  ...
  test('Workflow state restores after page reload', async ({ page }) => {
    ...
    await expect(page.locator('text=Workflow restored').first()).toBeVisible({ timeout: 30000 });
    await expect(page.locator('text=Is this section applicable').first()).toBeVisible({ timeout: 30000 });
  });
});
