...
async function openStudioStep(page: Page, docType: string) {
  ...
  const tabLabel =
    docType === "brs" ? "BRS" : docType === "srs" ? "SRS/SDD" : "Test Cases";
  const docTab = page
    .locator("button")
  ...
  const stepButton = page
    .locator("button", {
      has: page
        .locator("div")
        .filter({ hasText: /^(NOT STARTED|IN PROGRESS|HAS FINDINGS)$/i })
        .first(),
    })
    .first();
...
test.describe("T-01 Admin user — full access", () => {
  ...
  test("Admin Settings button visible", async ({ page }) => {
    await expect(
      page.getByRole("link", { name: /Admin Settings/i }),
    ).toBeVisible();
  });
  ...
  test("Admin Settings page loads config", async ({ page }) => {
    ...
    await expect(page.locator("text=Platform Governance").first()).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("Can approve BRS, SRS and Test Case sections", async ({ page }) => {
    for (const docType of ["brs", "srs", "testcase"]) {
      ...
      await expect(
        page.getByRole("button", { name: /^(Approve|Approve & Advance)$/i }),
      ).toBeVisible();
    }
  });
...
test.describe("T-02 BA Lead — BRS only, no admin", () => {
  ...
  test("Can approve BRS sections", async ({ page }) => {
    ...
    await expect(
      page.getByRole("button", { name: /Approve & Advance/i }),
    ).toBeVisible();
  });
...
test.describe("T-04 QA Lead — Test Cases only", () => {
  ...
  test("Cannot complete BRS", async ({ page }) => {
    ...
    await openStudioStep(page, "brs");
    await expect(
      page.getByRole("button", { name: /Complete BRS/i }),
    ).not.toBeVisible();
  });
});
...
test.describe("T-07 Token refresh", () => {
  ...
  test("Authenticated API calls work without 401 within token lifetime", async ({
    page,
  }) => {
