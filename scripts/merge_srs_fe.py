...
def extract_traces_to(content):
    """Extract SR-FE-xxx -> BR-xxx mappings from markdown table 'Traces To' column."""
    traces = []
    lines = content.split('\n') if content else []
    in_table = False
    traces_col_idx = -1
        ...
        if '|' in line and '---' not in line:
            ...
            if not in_table:
                for i, cell in enumerate(cells):
                    if 'traces' in cell.lower() and 'to' in cell.lower():
                        traces_col_idx = i
                        in_table = True
                    if cell.startswith('SR-FE'):
                        sr_col_idx = i

            if in_table and traces_col_idx >= 0 and sr_col_idx >= 0:
                if sr_col_idx < len(cells) and traces_col_idx < len(cells):
                    sr_id = cells[sr_col_idx]
                    br_ref = cells[traces_col_idx]
                    if sr_id.startswith('SR-FE') and br_ref.startswith('BR-'):
                        traces.append(f"{sr_id} -> {br_ref}")
