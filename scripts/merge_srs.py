#!/usr/bin/env python3
"""
AetherSpec — SRS-BE Document Merger
Deterministically assembles the final SRS-BE document from approved sections.
No LLM involved. 100% accurate. 0% hallucination.

Usage:
    python3 merge_srs.py --project-id prj-001 --doc-id doc-001 --db-url "..."
...
Output:
    ...
    {project_id}/srs-be/appendices/A-rtm.md         (requirements traceability matrix)
    {project_id}/srs-be/appendices/B-approval.md    (approval record)
    {project_id}/srs-be/appendices/C-history.md     (change history placeholder)
    {project_id}/srs-be/appendices/D-revisions.md   (draft revision log)
"""
...
def build_rtm(all_ids):
    ...
    for id_str in all_ids:
        if id_str.startswith("SR-BE-"):
            id_type = "Backend Functional Requirement"
        elif id_str.startswith("NFR-BE-"):
            id_type = "Non-Functional Requirement"
        elif id_str.startswith("DATA-BE-"):
            id_type = "Data Requirement"
        elif id_str.startswith("INT-BE-"):
            id_type = "Integration Requirement"
        elif id_str.startswith("SEC-BE-"):
            id_type = "Security Requirement"
        elif id_str.startswith("RULE-BE-"):
            id_type = "Backend Business Rule"
        elif id_str.startswith("ASSUMP-BE-"):
            id_type = "Assumption"
        elif id_str.startswith("RISK-BE-"):
            id_type = "Risk"
        elif id_str.startswith("UC-BE-"):
            id_type = "Backend Use Case"
        elif id_str.startswith("AC-BE-"):
            id_type = "Acceptance Criterion"
        else:
            id_type = "Unknown"
...
def build_main_srs(sections_content, all_ids, all_sources, output_name):
    """Build the main SRS-BE document."""
    ...
    lines = [
        "---",
        f"id: {srs_id}",
        "title: Software Requirements Specification — Backend",
        "version: v1.0.0",
        "status: SIGNED_OFF",
        f"created: {now}",
        f"approved: {now}",
...
def main():
    parser = argparse.ArgumentParser(
        description="Merge approved SRS-BE sections into a final document"
    )
    ...
    parser.add_argument(
        "--output-name",
        default="SRS-BE-001.md",
        help="Name of the merged SRS-BE file (used for frontmatter id)",
    )
    ...
    appendices = [
        ("srs-be/appendices/A-rtm.md", rtm_content),
        ("srs-be/appendices/B-approval.md", approval_content),
        ("srs-be/appendices/C-history.md", history_content),
        ("srs-be/appendices/D-revisions.md", revision_content),
    ]
    ...
    print("\n✅ SRS-BE merged successfully!")
    print(f"   Main document: {args.project_id}/{main_path}")
    print(f"   Appendices: 4 files in {args.project_id}/srs-be/appendices/")
