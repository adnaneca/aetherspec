#!/usr/bin/env python3
"""
AetherSpec — Frontend Backlog Generator
Deterministically extracts backlog items from the merged SRS-FE document.
No LLM involved. No agents. Pure extraction.

Usage:
    python3 generate_backlog_fe.py --project-id prj-001 --doc-id doc-002 --db-url "..." --generated-by admin
...
Output:
    {project_id}/backlog-fe/backlog-fe-001.md
"""
...
CATEGORY_MAP = {
    "SR-FE": "Functional",
    "NFR-FE": "Non-Functional",
    "UI-FE": "UI/UX",
    "INT-FE": "Interaction",
    "CONSTR-FE": "Constraint",
    "RULE-FE": "Business Rule",
    "ASSUMP-FE": "Assumption",
    "RISK-FE": "Risk",
}
...
def fetch_merged_srs_fe_path(conn, doc_id):
    """Return the minio_path of the merged SRS-FE document if it exists."""
    with conn.cursor() as cur:
        ...
        doc = cur.fetchone()
        if doc:
            return f"{doc['project_id']}/output/SRS-FE-001.md"
        return None
...
def extract_trace_targets(trace_text):
    """Extract BR-xxx IDs from a Traces-To cell."""
    if not trace_text:
        return []
    ids = re.findall(r"\bBR-\d+\b", trace_text)
    return ids


def extract_api_contracts(row, headers):
    """For INT-FE rows, extract INT-BE-xxx contract references from any cell."""
    contracts = []
    lower_headers = [h.lower() for h in headers]
    # Look in columns likely to hold the backend contract reference.
    candidate_names = ["int-be", "api", "contract", "backend api", "consumes", "calls"]
    candidate_indices = {
        i for i, h in enumerate(lower_headers)
        if any(c in h for c in candidate_names)
    }
    # Also scan the raw row itself for any INT-BE references.
    for i, cell in enumerate(row):
        if i in candidate_indices or re.search(r"INT-BE-\d+", cell or ""):
            contracts.extend(re.findall(r"INT-BE-\d+", cell or ""))
    return sorted(set(contracts))


def generate_story(requirement_text, category):
    """Generate a simple draft user story from a frontend requirement."""
    text = re.sub(r"\*\*|__|\*|_", "", requirement_text)
    text = re.sub(r"^\s*The frontend\s+", "", text, flags=re.IGNORECASE)
    text = re.sub(r"^\s*The frontend application\s+", "", text, flags=re.IGNORECASE)
    ...
    return f"As a user, I want {text.lower()} so that the frontend fulfills the {category.lower()} requirement."


def generate_backlog_fe_document(project_id, doc_id, generated_by, requirements):
    """Build the markdown backlog-fe document and summary."""
    categories = {}
    priorities = {}

    lines = [
        f"# Backlog — Frontend — {project_id}",
        "",
        f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"**Generated By:** {generated_by}",
        f"**Source Document:** {doc_id} (SRS-FE)",
    ...
    for i, req in enumerate(requirements, 1):
        ...
        api_contracts = ", ".join(req["api_contracts"]) if req["api_contracts"] else "N/A"
        ...
        lines.extend([
            ...
            f"**API Contracts (INT-BE):** {api_contracts}",
            f"**Effort Hint (Rough):** {effort}",
            "",
            f"**Story:** {story}",
            "",
            f"**Source Section:** {req.get('source_section') or 'SRS-FE'}",
            "",
            "---",
            "",
        ])
...
def main():
    parser = argparse.ArgumentParser(description="Generate frontend backlog from approved SRS-FE")
    ...
    parser.add_argument("--output-name", default="backlog-fe-001.md", help="Output backlog file name")
    ...
    minio_path = fetch_merged_srs_fe_path(conn, args.doc_id)
    if not minio_path:
        print("ERROR: Could not determine merged SRS-FE path", file=sys.stderr)
        sys.exit(1)

    key = minio_object_key(args.project_id, minio_path)
    content = fetch_object(minio_client, args.project_id, key)
    if content is None:
        print(f"ERROR: Could not fetch merged SRS-FE from {minio_path}", file=sys.stderr)
        sys.exit(1)
    ...
    for row in rows:
        req_id = row["id"]
        match = re.match(r"^([A-Z]+-FE)-\d+\b", req_id)
        ...
        source = row.get("source") or "SRS-FE"
        api_contracts = extract_api_contracts(row.get("raw_row", []), row.get("table_headers", []))
    ...
    if not requirements:
        print("ERROR: No requirements found in merged SRS-FE", file=sys.stderr)
        sys.exit(1)
    ...
    backlog_content = generate_backlog_fe_document(
        args.project_id, args.doc_id, args.generated_by, requirements
    )

    backlog_path = f"backlog-fe/{args.output_name}"
    ...
    print(f"✅ Frontend backlog generated: {args.project_id}/{backlog_path}")
